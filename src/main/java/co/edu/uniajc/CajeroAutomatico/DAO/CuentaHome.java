...
import co.edu.uniajc.CajeroAutomatico.model.Cuenta;

/**
 * Home object for domain model class Cuenta.
 * @see co.edu.uniajc.CajeroAutomatico.Daos.Cuenta
...
public class CuentaHome {
	...
	public Cuenta findById(java.lang.Integer id) {
		log.debug("getting Cuenta instance with id: " + id);
		try {
			Cuenta instance = (Cuenta) sessionFactory.getCurrentSession()
					.get("co.edu.uniajc.CajeroAutomatico.Daos.Cuenta", id);
			if (instance == null) {
	...
	public List findByExample(Cuenta instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("co.edu.uniajc.CajeroAutomatico.Daos.Cuenta").add(Example.create(instance)).list();
